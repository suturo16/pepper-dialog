...
class SpeechRecognitionWrapper(ALModule):
    ...
    def __init__(self):
		...
		self.PATHTONAOQIPACKAGE =rospy.get_param("PATHTONAOQIPACKAGE", "../NAOqi")
		self.NAOQIPACKAGENAME =rospy.get_param("NAOQIPACKAGENAME", "suturo16-0.0.0.pkg")
		self.PATHTOBEHAVIOR =rospy.get_param("PATHTOBEHAVIOR", "behavior_1")
		self.NAOQIPACKAGEUUID =rospy.get_param("NAOQIPACKAGEUUID", "suturo16")
		...
		self.packageManager=self.memory.session().service('PackageManager')
		#self.packageManager.install(os.path.join(self.PATHTONAOQIPACKAGE,self.NAOQIPACKAGENAME))
		...
		self.al.switchFocus(os.path.join(self.NAOQIPACKAGEUUID,self.PATHTOBEHAVIOR))        
		...
		self.memory.subscribeToEvent(
	        ...
	        rospy.loginfo("Speech recognizer started successfully: ")
	except Exception,e:
		      rospy.logwarn("Speech recognizer did not start properly: "+str(e))
